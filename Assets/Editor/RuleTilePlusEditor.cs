...
namespace UnityEditor {
    [CustomEditor(typeof(RuleTilePlus), true)]
    ...
    public class RuleTileEditor : Editor {
        ...
        public RuleTilePlus tile => target as RuleTilePlus;
        ...
        public virtual void OnEnable() {
            m_ReorderableList = new ReorderableList(tile.m_TilingRules, typeof(RuleTilePlus.TilingRule), true, true, true, true);
        ...
        public virtual void RuleOnGUI(Rect rect, Vector3Int position, int neighbor) {
            switch (neighbor) {
                case RuleTilePlus.TilingRule.Neighbor.This:
                    GUI.DrawTexture(rect, arrows[GetArrowIndex(position)]);
                    break;
                case RuleTilePlus.TilingRule.Neighbor.NotThis:
        ...
        public virtual void RuleTransformOnGUI(Rect rect, RuleTilePlus.TilingRule.Transform ruleTransform) {
            switch (ruleTransform) {
                case RuleTilePlus.TilingRule.Transform.Rotated:
                    GUI.DrawTexture(rect, autoTransforms[0]);
                    break;
                case RuleTilePlus.TilingRule.Transform.MirrorX:
                    GUI.DrawTexture(rect, autoTransforms[1]);
                    break;
                case RuleTilePlus.TilingRule.Transform.MirrorY:
                    GUI.DrawTexture(rect, autoTransforms[2]);
                    break;
                case RuleTilePlus.TilingRule.Transform.Fixed:
                    GUI.DrawTexture(rect, autoTransforms[3]);
                    break;
                case RuleTilePlus.TilingRule.Transform.MirrorXY:
        ...
        public virtual void SpriteOnGUI(Rect rect, RuleTilePlus.TilingRuleOutput tilingRule) {
            tilingRule.m_Sprites[0] = EditorGUI.ObjectField(rect, tilingRule.m_Sprites[0], typeof(Sprite), false) as Sprite;
        }

        public void RuleInspectorOnGUI(Rect rect, RuleTilePlus.TilingRuleOutput tilingRule) {
            float y = rect.yMin;
            ...
            tilingRule.m_Output = (RuleTilePlus.TilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
            y += k_SingleLineHeight;

            if (tilingRule.m_Output == RuleTilePlus.TilingRule.OutputSprite.Animation) {
            ...
            if (tilingRule.m_Output == RuleTilePlus.TilingRule.OutputSprite.Random) {
                ...
                tilingRule.m_RandomTransform = (RuleTilePlus.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RandomTransform);
                y += k_SingleLineHeight;
            }

            if (tilingRule.m_Output == RuleTilePlus.TilingRule.OutputSprite.Animation || tilingRule.m_Output == RuleTilePlus.TilingRule.OutputSprite.Random) {
            ...
            } else if (tilingRule.m_Output == RuleTilePlus.TilingRule.OutputSprite.WeightedRandom) {
                // Size label
                GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Size");
                ...
                // Sprite & weight label
                int halfWidth = Mathf.FloorToInt((rect.width - k_LabelWidth) / 2);
                GUI.Label(new Rect(rect.xMin + k_LabelWidth, y, halfWidth, k_SingleLineHeight), "Sprite");
                GUI.Label(new Rect(rect.xMin + k_LabelWidth + halfWidth, y, halfWidth, k_SingleLineHeight), "Weight");
                y += k_SingleLineHeight;

                // Fields
                for (int i = 0; i < tilingRule.m_Sprites.Length; i++) {
                    tilingRule.m_Sprites[i] = EditorGUI.ObjectField(new Rect(rect.xMin + k_LabelWidth, y, halfWidth, k_SingleLineHeight), tilingRule.m_Sprites[i], typeof(Sprite), false) as Sprite;
                    tilingRule.m_Weights[i] = EditorGUI.DelayedIntField(new Rect(rect.xMin + k_LabelWidth + halfWidth, y, halfWidth, k_SingleLineHeight), tilingRule.m_Weights[i]);
                    y += k_SingleLineHeight;
                }
            }
        }
        ...
        class RuleTileRuleWrapper {
            [SerializeField]
            public List<RuleTilePlus.TilingRule> rules = new List<RuleTilePlus.TilingRule>();
        }

        [MenuItem("CONTEXT/RuleTilePlus/Copy All Rules")]
        ...
        [MenuItem("CONTEXT/RuleTilePlus/Paste Rules")]
