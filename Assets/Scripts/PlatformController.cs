...
public class PlatformController : RaycastController {
    ...
    void MovePassengers(bool beforeMovePlatform) {
        foreach (PassengerMovement passenger in passengerMovement) {
            ...
            if (passenger.moveBeforePlatform == beforeMovePlatform && !passengerDictionary[passenger.transform].collisions.fallingThroughPlatform) {
    ...
    void CalculatePassengerMovement(Vector3 velocity) {
        ...
        if (velocity.x != 0) {
            ...
            for (int i = 0; i < horizontalRayCount; i++) {
                ...
                Physics2D.SyncTransforms();
                RaycastHit2D[] hits = Physics2D.RaycastAll(rayOrigin, Vector2.right * directionX, rayLength, passengerMask);
        ...
        if (directionY == -1 || velocity.y == 0 && velocity.x != 0) {
            ...
            for (int i = 0; i < verticalRayCount; i++) {
                ...
                Physics2D.SyncTransforms();
                RaycastHit2D[] hits = Physics2D.RaycastAll(rayOrigin, Vector2.up, rayLength, passengerMask);
